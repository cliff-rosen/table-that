/**
 * Categorization Prompt Form
 *
 * Allows users to customize the prompt used for article categorization.
 * Similar to ContentEnrichmentForm but focused on a single prompt type.
 *
 * Features:
...
export default function CategorizationPromptForm({ streamId, stream }: CategorizationPromptFormProps) {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            console.error('Failed to load categorization config:', err);
            setError('Failed to load categorization configuration');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleTest = async () => {
        ...
        try {
            ...
            console.error('Error testing categorization prompt:', err);
                    ...
                    <>
                        ...
                        {testResult?.llm_response && (
                            <div className="flex flex-col">
                                ...
                                <div className={`bg-green-50 dark:bg-green-900/20 rounded-lg p-4 border border-green-200 dark:border-green-800 overflow-y-auto resize-y ${isFullMode ? 'min-h-[150px] flex-1' : 'min-h-[100px]'}`}>
                                    <pre className="text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap font-mono">
                                        {testResult.llm_response}
                                    </pre>
                                </div>
                                {testResult.parsed_category_id && (
                                    <div className="mt-2 flex items-center gap-2">
                                        <span className="text-xs text-gray-500">Assigned Category:</span>
            ...
            <div className="flex items-center justify-between flex-shrink-0 mb-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        Customize how articles are assigned to categories
                    </p>
                </div>
                <div className="flex items-center gap-2">
                    {isUsingDefaults && (
                    ...
                    <button
                        ...
                        className="px-4 py-2 text-sm bg-green-600 text-white hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed rounded-md transition-colors"
                    >
                ...
                <div className="flex-1 min-w-0 flex flex-col min-h-0 overflow-y-auto">
                    ...
                    <div className="flex flex-col flex-1 min-h-0 mb-4">
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 flex-shrink-0">
                            User Prompt Template
                            <span className="text-gray-400 font-normal ml-2">(Use slugs like {'{title}'}, {'{categories_json}'})</span>
                        </label>
                        <textarea
                        ...
                        {testMode === 'paste' && (
                            <div>
                                <textarea
                                    ...
                                    rows={4}
                                    placeholder='{"title": "...", "abstract": "...", "journal": "...", "publication_date": "Jan 2024", "categories_json": "[...]"}'
