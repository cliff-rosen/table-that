...
**Date:** 2026-03-04
**Test User:** qa_test_20260304_2045@test.example.com
**Base URL:** http://localhost:5174
**Browser:** Playwright (Chromium)
**Purpose:** Pre-deployment validation after cross-page streaming concurrency fix
...
| 5 | MINOR | PASS | PASS | [D] Priority values all "High" despite AI text claiming a mix (see #2) |
| 6 | n/a | PASS | PASS | Session persistence, theme toggle, console audit clean |
| 7 | PASS | PASS | PASS | Context correctly switches between tables; conversations isolated; no duplicate creation messages |

## Key Validation: Cross-Page Streaming Fix

The primary purpose of this run was to validate the concurrency fix (moving `sendMessage` from TablesListPage to TableViewPage via React Router state). Results:

- **Table creation from tables list**: Schema proposal accepted, navigated to `/tables/220`, creation message sent correctly from destination page
- **No duplicate streaming**: `[User accepted and created "Job Applications"]` message appeared only once in the QA Pets creation (Phase 7a), confirming the StrictMode ref guard works
- **Context isolation**: Navigating between Job Applications and QA Pets correctly loads separate conversations
- **loadForContext**: Works correctly after navigation; no stale `lastLoadedPageRef` issues observed

**Note:** The Job Applications table (Phase 2) still shows duplicate creation messages from a pre-fix run. These are historical DB artifacts and will not recur with the fix in place.
...
| Stays on current page after registration | PASS | 1b-alt | Remained on /tables/220 |
...
| Apply creates table, card disappears | PASS | 2 | Navigated to /tables/220 |
...
| New column appears with placeholder | PASS | 4 | Priority column with empty values |
...
| Checklist Item | Result | Phase | Notes |
|---------------|--------|-------|-------|
| Second table created via chat proposal | PASS | 7a | QA Pets with Name, Species, Age |
| AI identifies correct table after creation | PASS | 7b | "QA Pets" with correct columns |
| Context switches after navigation to first table | PASS | 7c | "Job Applications" with 8 rows, 7 columns |
| No cross-table conversation leakage | PASS | 7c | Chat panel shows Job Applications history, not QA Pets |
| Single creation message (StrictMode fix) | PASS | 7a | Only one "[User accepted and created "QA Pets".]" message |

## Issues Found

| # | Severity | Layer | Phase | Description | Evidence |
|---|----------|-------|-------|-------------|----------|
| 1 | Low | D | 2 | Duplicate creation messages visible in Job Applications chat from pre-fix run. These are historical DB artifacts — the fix was validated by the QA Pets creation in Phase 7a which shows only one message. | qa-7c-context-switch.png — two identical "[User accepted and created...]" messages from 2:55:17 AM |
| 2 | Low | D | 5 | Enrichment assigned "High" to all 8/8 rows. AI's own explanation categorized applications into High/Medium/Low but the actual enrichment values are uniformly "High". Same issue as previous QA run. | qa-5-enriched.png — AI text says "3 high-priority, 4 medium, 1 low" but all cells show "High" |
| 3 | Cosmetic | P | 6 | loadForContext 404 errors logged to console.error when no conversation exists yet. These are expected (lookup-only) but noisy — could be console.log level. | Console audit: 6 occurrences of "Failed to load chat for context: tables_list" |
...
| qa-1b-alt-guest.png | 1b-alt | Guest flow: schema proposal card |
| qa-2-table-created.png | 2 | Table created with 3 sample rows, suggestion chips |
| qa-3-data-proposal.png | 3 | Data proposal with 5 new rows |
| qa-3-populated.png | 3 | 8 rows after applying data proposal |
...
| qa-5-enrichment-complete.png | 5 | DataProposalActionBar: 8 updates |
...
- 404 on `/api/chats/by-context?current_page=tables_list` (no conversation exists yet — expected for lookup-only pattern)
...
2. **Low priority**: The enrichment strategy for the Priority column assigned "High" to everything. The `computation` strategy (no web search needed) would be more appropriate for deriving values from existing row data. This is an AI decision quality issue, not a tool bug.
...
4. **Validated**: The cross-page streaming concurrency fix is working correctly. Table creation from the tables list no longer causes stale context or duplicate messages. Safe to deploy.
